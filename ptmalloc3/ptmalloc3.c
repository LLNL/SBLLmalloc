...
//  fprintf(stderr, "%%%% ptmalloc3: malloc initialized\n");
}

/*------------------------ Public wrappers. --------------------------------*/
#include <internal-routines.h>
#ifdef ENABLE_SHM_MALLOC
#include <Globals.h>
#endif /*ENABLE_SHM_MALLOC*/
...
#ifdef ENABLE_SHM_MALLOC
	void* ret_ptr;
	// FIXME: change 4096 to PAGE_SIZE
	if(bytes >= 4096){
		ret_ptr = ShmMallocWrapper(bytes);
		if(ret_ptr)
			return ret_ptr;
	}
#endif /*ENABLE_SHM_MALLOC*/
...
{
#ifdef ENABLE_SHM_MALLOC
	if(ShmFreeWrapper(mem) < 0)
#endif /*ENABLE_SHM_MALLOC*/
		return internal_free(mem);
}
...
#ifdef ENABLE_SHM_MALLOC
	void * new_ptr = NULL;
	if (oldmem == NULL || bytes == 0) {         
		return internal_realloc(oldmem, bytes);
	}
	// FIXME if the region changes between shm-core and ptmalloc, need to handle it
	new_ptr = ShmReallocWrapper(oldmem, bytes);
	if(new_ptr)
		return new_ptr;
#endif /*ENABLE_SHM_MALLOC*/
	return internal_realloc(oldmem, bytes);
...
void*
internal_realloc(void* oldmem, size_t bytes)
{
  struct malloc_arena* ar_ptr;
...
public_vALLOc(size_t bytes)
{
	...
	// TODO: implement
#endif /*ENABLE_SHM_MALLOC*/
	return internal_valloc(bytes);
}
void*
internal_valloc(size_t bytes)
{
  struct malloc_arena* ar_ptr;
  void *p;
...
#ifdef ENABLE_SHM_MALLOC
	  void *ptr = NULL;
	  if(n_elements * elem_size >= 4096){
		  ptr = ShmMallocWrapper(n_elements * elem_size);
	  }

	  if(ptr){
		  memset(ptr, 0, ((n_elements * elem_size+4095)>>12) <<12);
		  return ptr;
	  }
#endif /*ENABLE_SHM_MALLOC*/
	return internal_calloc(n_elements, elem_size);
}
void*
internal_calloc(size_t n_elements, size_t elem_size)
{
  struct malloc_arena* ar_ptr;
  size_t bytes, sz;
  void* mem;
...
public_iCALLOc(size_t n, size_t elem_size, void* chunks[])
{
	...
	// TODO: implement
#endif /*ENABLE_SHM_MALLOC*/
	return internal_icalloc(n, elem_size, chunks);
}

void**
internal_icalloc(size_t n, size_t elem_size, void* chunks[])
{
  struct malloc_arena* ar_ptr;
  void** m;
...
public_iCOMALLOc(size_t n, size_t sizes[], void* chunks[])
{
	...
	// TODO: implement
#endif /*ENABLE_SHM_MALLOC*/
	return internal_icomalloc(n, sizes, chunks);
}

void**
internal_icomalloc(size_t n, size_t sizes[], void* chunks[])
{
  struct malloc_arena* ar_ptr;
  size_t* m_sizes;
  size_t i;
...
public_mTRIm(size_t s)
{
	...
	// TODO: implement
#endif /*ENABLE_SHM_MALLOC*/
	return internal_mtrim(s);
}

int
internal_mtrim(size_t s)
{
  int result;
...
public_mUSABLe(void* mem)
{
	...
	// TODO: implement
#endif /*ENABLE_SHM_MALLOC*/
	return internal_musable(mem);
}

size_t
internal_musable(void* mem)
{
  if (mem != 0) {
    mchunkptr p = mem2chunk(mem);
    if (cinuse(p))
...
public_mALLOPt(int p, int v)
{
	...
	// TODO: implement
#endif /*ENABLE_SHM_MALLOC*/
	return internal_mallopt(p,v);
}

int
internal_mallopt(int p, int v)
{
  int result;
...
public_mSTATs(void)
{
	...
	// TODO: implement
#endif /*ENABLE_SHM_MALLOC*/
	return internal_mstats();
}

void
internal_mstats(void)
{
  int i;
  struct malloc_arena* ar_ptr;
...
internal_footprint(void)
{
	...
	for (i=0, ar_ptr = &main_arena;; ++i) {
		...
		mem_footprint += mspace_footprint(msp);
		ar_ptr = ar_ptr->next;
		if (ar_ptr == &main_arena)
			break;
	}
